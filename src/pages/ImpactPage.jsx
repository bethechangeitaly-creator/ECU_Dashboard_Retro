    ...
    >
        {objectives.map((obj, idx) => (
            <motion.div
                ...
                className={`bg-white rounded-xl shadow-sm border-l-4 border-${obj.color}-500 p-6`}
            >
                <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                            <span className={`px-3 py-1 bg-${obj.color}-100 text-${obj.color}-700 text-xs font-bold rounded-full`}>
                                DAY {obj.day}
                            </span>
                            <span className="text-sm text-slate-500 font-medium">{obj.body_part}</span>
                        </div>
                        <h3 className="text-2xl font-bold text-slate-900 mb-2">{obj.title}</h3>
                ...
                {/* Target */}
                <div className={`bg-${obj.color}-50 rounded-lg p-4 mb-4`}>
                    <div className="flex items-start space-x-3">
                        <Target className={`text-${obj.color}-600 flex-shrink-0 mt-1`} size={20} />
                        <div>
                            <p className="text-xs font-semibold text-slate-600 uppercase tracking-wider mb-1">Target</p>
                            <p className={`text-${obj.color}-900 font-semibold`}>{obj.target}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-slate-50 rounded-lg p-4">
                        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Baseline</p>
                        <p className="text-sm text-slate-700">{obj.metrics.baseline}</p>
                    </div>
                    <div className="bg-slate-50 rounded-lg p-4">
                        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Measurement</p>
                        <p className="text-sm text-slate-700">{obj.metrics.measurement}</p>
                    </div>
                    <div className="bg-slate-50 rounded-lg p-4">
                        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Success</p>
                        <p className="text-sm text-slate-700">{obj.metrics.success_criteria}</p>
                    </div>
                </div>

                {/* Methods */}
                <div className="mt-4">
                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Methods Used</p>
                    <div className="flex flex-wrap gap-2">
                        {obj.methods.map((method, midx) => (
                            <span key={midx} className="px-3 py-1 bg-slate-100 text-slate-700 text-sm rounded-lg">
                                {method}
                            </span>
                        ))}
                    </div>
...
const GoalsTab = ({ goals }) => {
    ...
    const colorClasses = {
        rose: { top: 'bg-rose-500', icon: 'bg-rose-100 text-rose-600', badge: 'bg-rose-600' },
        violet: { top: 'bg-violet-500', icon: 'bg-violet-100 text-violet-600', badge: 'bg-violet-600' },
        blue: { top: 'bg-blue-500', icon: 'bg-blue-100 text-blue-600', badge: 'bg-blue-600' },
        amber: { top: 'bg-amber-500', icon: 'bg-amber-100 text-amber-600', badge: 'bg-amber-600' },
        pink: { top: 'bg-pink-500', icon: 'bg-pink-100 text-pink-600', badge: 'bg-pink-600' },
        emerald: { top: 'bg-emerald-500', icon: 'bg-emerald-100 text-emerald-600', badge: 'bg-emerald-600' },
    };
            ...
            {priorities.map((priority, idx) => {
                ...
                return (
                    ...
                    >
                        <div className="flex items-start justify-between mb-4">
                            <div className="flex-1">
                                <h3 className="text-2xl font-bold text-slate-900 mb-3">{priority.name}</h3>
                                <p className="text-slate-700 leading-relaxed">
                            ...
                            <div className={`ml-6 flex-shrink-0 ${colors.bg} rounded-xl p-4 text-center`}>
                                ...
                                <p className="text-xs text-slate-600 font-semibold mt-1">Impact</p>
                            </div>
                        </div>
                        {/* Progress Bar */}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {Object.entries(indicators.direct_participants).map(([key, value]) => (
                    <div key={key} className="bg-indigo-50 rounded-lg p-4 text-center">
                        ...
                        <p className="text-xs text-slate-600 font-semibold capitalize">{key.replace(/_/g, ' ')}</p>
                    </div>
                ))}
            </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {Object.entries(indicators.expected_outcomes).map(([key, value]) => {
                    ...
                    return (
                        <div key={key} className="flex items-start space-x-3 bg-slate-50 rounded-lg p-4">
                            <CheckCircle2 size={20} className="text-emerald-600 flex-shrink-0 mt-0.5" />
                            <div>
                                <p className="text-sm font-semibold text-slate-700 capitalize mb-1">
                                    {key.replace(/_/g, ' ')}
                                </p>
                                <p className="text-2xl font-bold text-emerald-600 mb-1">{value}</p>
